...
if state:
    ...
    reactors = {
        1: {"co2": 0, "mode": "PID",   "iae": state["r1_iae"], "ise": state["r1_ise"], "itae": state["r1_itae"]},
        2: {"co2": 0, "mode": "ONOFF", "iae": state["r2_iae"], "ise": state["r2_ise"], "itae": state["r2_itae"]}
    }
else:
    start = time.time()
    reactors = {
        1: {"co2": 0, "mode": "PID",   "iae": 0, "ise": 0, "itae": 0},
        2: {"co2": 0, "mode": "ONOFF", "iae": 0, "ise": 0, "itae": 0}
    }
...
try:
    while True:
        ...
        light_state = 1 if cycle_time < (LIGHT_ON_HOURS * 3600) else 0
        ...
        for rid, r in reactors.items():
            ...
            if r["mode"] == "PID":
                ...
                if action is not None:
                    if action != r["co2"]:
                        insert_event(rid, "CO2", "Activated" if action == 1 else "Deactivated", "ON/OFF control", "running")
                    r["co2"] = action
                    if not TEST_MODE:
                        set_co2(rid, r["co2"])
